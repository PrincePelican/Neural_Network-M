...
conv3Din::conv3Din(unsigned _kernelSize, unsigned _kernelNumber, unsigned _matrixSize, std::vector<float**>* _matrix_in, std::vector<float**>* _out, std::vector<float**>* _error, std::vector<float**>* _error_out, bool _flat, float* _flatten, Active_functions* _function)
{
...
void conv3Din::create_errorMatrix(std::vector<float**>* error_in)
{
			...
			for (int j = starty; j < endy; ++j) {
				...
				for (int w = wp; w < limitw; ++w) {
					for (int k = kp; k < limitk; ++k) {
						(*error_out)[x][j - starty][i - startx] += kernels[x][w][k] * (*error_in)[x][w+j][k+i];
					}
				}
			}
		}
